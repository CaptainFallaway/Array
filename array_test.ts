...
import { AllocatedStack, InvalidRangeError, OutOfBoundsError, SizeTooSmallError } from "./array.ts";

// Helper to fill a vector from values array
function fill(v: AllocatedStack, values: number[]) {
...
Deno.test("push throws when full (boundary)", () => {
	const size = 5;
	const v = new AllocatedStack(size);
...
Deno.test("constructor -> throws when size is 0", () => {
	assertThrows(() => new AllocatedStack(0), SizeTooSmallError);
});

Deno.test("constructor -> throws when size is 1", () => {
	assertThrows(() => new AllocatedStack(1), SizeTooSmallError);
});

Deno.test("constructor -> accepts size of 2 (minimum valid)", () => {
	const v = new AllocatedStack(2);
	assertEquals(v.length, 0);
	assertEquals(v.isEmpty, true);
});

Deno.test("constructor -> large size allocation", () => {
	const v = new AllocatedStack(2 ** 16);
	assertEquals(v.length, 0);
	assertEquals(v.isEmpty, true);
});
...
Deno.test("merge -> large array", () => {
	const v = new AllocatedStack(100);
	fill(v, [1, 2, 3]);
